...
PETMAST.PVACOUNT FROM PETMAST JOIN ARCUSTO ON (ARCUSTO.CUSTNO = PETMAST.CUSTNO) WHERE ARCUSTO.INACTIVE <> 1 AND DATE_SUB(NOW(),INTERVAL 4 MONTH) < PDOB AND PETMAST.PLASTDATE >= '$startdate[0]' AND 
PETMAST.POTHDAT <= '$enddate[0]' AND (PETMAST.PDEAD + PETMAST.PMOVED = 0) AND INSTR(UPPER(PETMAST.PDATA),'NO REM') = 0  AND INSTR(UPPER(ARCUSTO.COMMENT),'NO REM') = 0 AND (".$species.") $skipref $namerange";
$RECALL = mysqli_query($tryconnection, $query_RECALL) or die(mysqli_error($mysqli_link));
...
if ($xtable == 'PUPKIT') {
$xsearch = mysqli_real_escape_string($mysqli_link, "Vaccinated between $_POST[startdate] - $_POST[enddate]");
}
...
<title>PUPKIT SEARCH RESULTS</title>
<!-- InstanceEndEditable -->
...
<?php echo $openwin; ?>
	if (document.pupkit_search_results.report[3].checked == true){
	document.getElementById('pcds').style.display="";
	}
	else {
	document.getElementById('pcds').style.display="none";
	}
document.pupkit_search_results.xsubtype[0].checked = true;
}

function showtemplate(){
var report=document.search_results.report;
	if (document.pupkit_search_results.report[3].checked == true || document.pupkit_search_results.report[4].checked == true){
	document.getElementById('pcds').style.display="";
	}
	else {
	document.getElementById('pcds').style.display="none";
...
<form action="PUPKIT_SEARCH_RESULTS_PRINT.php" name="search_results" method="get" target="_blank">
